...
class Gateway(metaclass=AutodocABCMeta):
    ...
    def __del__(self):
        ...
        for name in server_names:
            try:
                process = self._processes[name]["process"]
                process.terminate()
    ...
    def init_sse_server(self, server_name: str):
        ...
        self._processes[server_name]["routes"] = self._build_routes(server_name)
    ...
    def _build_routes(self, server_name: str) -> List:
        """
        Builds Starlette routes.
        ...
        """
        assert server_name in self._processes, f"Server {server_name} is not initialized."
    ...
    def build_starlette_app(self, debug: bool = True) -> Starlette:
        """
...
def main(port: int, config: str):
    ...
    app = gateway.build_starlette_app()
